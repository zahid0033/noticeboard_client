...
export default function EditNoticeSet({ match }) {
    ...
    const getNoticeSet = useCallback(async () => {
        try {
            const { data } = await axios.get(`/admin/getnoticeset/${match.params.id}`)
    ...
    const updateNoticeSet = async (values) => {
        try {
            const { data } = await axios.put(`/admin/updatenoticeset/${match.params.id}`, values)
            if (data.success) {
                ...
                <Col style={{ marginTop: "20px" }}>
                    <Form onSubmit={handleSubmit(updateNoticeSet)}>
                        ...
                        <Form.Group>
                            ...
                            <Form.Control defaultValue={noticeSet?.name} disabled={!editNoticeSet} name="name" ref={register({
                                required: "You must input a notice set name",
                            })} />
                        </Form.Group>
                        <Form.Group>
                            <Form.Label>Select notice display type</Form.Label>
                            <Form.Control disabled={!editNoticeSet} as="select" defaultValue={noticeSet?.viewtype} name="viewtype" ref={register({
                        ...
                        {watch('viewtype') !== 'singleimage' &&
                            <Form.Group>
                                ...
                                <Form.Control disabled={!editNoticeSet} defaultValue={noticeSet?.interval} name="interval" type="number" ref={register({
                        ...
                        <Form.Group>
                            {materialloading ? <Spinner animation="border" />
                                ...
                                <>
                                    ...
                                    <Form.Control disabled={!editNoticeSet} onChange={e => setMaterialQuery(e.target.value)} placeholder="Search materials here" />
                                    <div style={{ maxHeight: "200px", overflow: 'auto' }}>
                                        {materials?.filter(material => {
                                            ...
                                            <Form.Check
                                                ...
                                                type="checkbox"
                                                id="materials"
                                                defaultChecked={noticeSet?.materials?.some(ns => ns._id === material._id)}
                                                name="materials"
                                                label={material.name}
                                                value={material._id}
                                                ref={register}
                        ...
                        {editNoticeSet ? (
                            <>
                                <Button type="button" onClick={() => setEditNoticeSet(false)}>Cancel</Button>
                                <Button type="submit">Update Notice Set</Button>
                            </>
                        ) : (
                                <>
                                    <Button type="button" onClick={() => setEditNoticeSet(true)}>Edit</Button>
                                </>
                            )}
                    </Form>
                </Col>
